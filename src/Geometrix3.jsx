...
function GeometrixLiveClient({ codigoSala, usuario, onExit }) {
    ...
    // Nombre: si hay usuario logado se usa directamente.
    // Si es invitado, se pide UNA SOLA VEZ y se guarda en localStorage para
    // evitar que React StrictMode (que monta dos veces en desarrollo) lo pregunte dos veces.
    const [myName] = useState(() => {
        if (usuario?.displayName) return usuario.displayName;
        const stored = localStorage.getItem('pikt_guest_name');
        if (stored) return stored;
        const nombre = window.prompt("¿Cuál es tu nombre?") || "Invitado";
        localStorage.setItem('pikt_guest_name', nombre);
        return nombre;
    });
