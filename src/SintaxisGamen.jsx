...
export default function SintaxisGame({ onExit, isHost, codigoSala, usuario, recurso }) {
    ...
    if (!idioma) return <PantallaIdioma onEspanol={()=>setIdioma('ES')} onFrances={()=>setIdioma('FR')} onExit={onExit}/>;
...
// ─────────────────────────────────────────────────────────────────────
function PantallaIdioma({ onEspanol, onFrances, onExit }) {
    return (
...
function SintaxisAppFR({ onExit }) {
    ...
    if (screen==='MODOS') return (
        <div style={g.container}>
            <div style={g.header}>
                <button onClick={()=>setScreen('NIVEL')} style={g.btnBack}><RotateCcw size={16}/> Volver</button>
                <span style={g.titulo}>🇫🇷 Syntaxe Française</span>
                <div style={{width:60}}/>
            </div>
            <div style={g.centerBox}>
                <div style={{...g.card,maxWidth:520}}>
                    <div style={{fontSize:'2.5rem',marginBottom:4}}>🇫🇷</div>
                    <h2 style={{...g.cardTitle,fontSize:'1.4rem'}}>
                        {(NIVELES_FR.find(n=>n.id===nivelSel)||{emoji:'📚',label:'Tous'}).emoji}&nbsp;
                        {(NIVELES_FR.find(n=>n.id===nivelSel)||{label:'Tous'}).label}
                    </h2>
                    <div style={{display:'flex',flexDirection:'column',gap:10,marginTop:16}}>
                        {[
                            {key:'LOCAL',emoji:'🎯',label:'Mode Libre',desc:"Entraîne-toi à ton rythme.",color:'#3498db',onClick:()=>setScreen('LOCAL')},
                            {key:'CONTRARRELOJ',emoji:'⏱️',label:'Contre-la-montre',desc:'Analyse le maximum de phrases en 3 minutes.',color:'#e74c3c',onClick:()=>setScreen('CONTRARRELOJ')},
                        ].map(m=>(
                            <button key={m.key} onClick={m.onClick}
                                style={{padding:'16px 20px',borderRadius:16,border:`2px solid ${m.color}`,background:'white',cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:14,transition:'all 0.15s',fontFamily:'inherit'}}
                                onMouseEnter={e=>e.currentTarget.style.background=`${m.color}15`}
                                onMouseLeave={e=>e.currentTarget.style.background='white'}>
                                <span style={{fontSize:'1.8rem'}}>{m.emoji}</span>
                                <div>
                                    <div style={{fontWeight:'bold',color:m.color,fontSize:'0.95rem'}}>{m.label}</div>
                                    <div style={{color:'#888',fontSize:'0.8rem'}}>{m.desc}</div>
                                </div>
                                <ChevronRight size={18} color={m.color} style={{marginLeft:'auto'}}/>
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div style={g.centerBox}>
                <div style={{...g.card,maxWidth:520}}>
                    ...
                    <p style={{color:'#666',fontSize:'0.88rem',marginBottom:20}}>
                        Sélectionne un niveau pour commencer à analyser des phrases.
                    </p>
                        ...
                        {NIVELES_FR.map(n=>(
                            <button key={String(n.id)} onClick={()=>setNivelSel(n.id)}
                                style={{
                                    ...
                                    background:nivelSel===n.id?n.color:'#f5f5f5',
                                    color:nivelSel===n.id?'white':'#444',
                                    border:`2px solid ${n.color}`,
                                }}>
                                {n.emoji} {n.label}
                                <div style={{fontSize:'0.68rem',opacity:.8,marginTop:2,fontWeight:'normal'}}>
                                    {FRASES_FR.filter(f=>!n.id||f.nivel===n.id).length} phrases
                                </div>
                            </button>
                        ))}
                    </div>
                    <button
                        onClick={()=>setScreen('MODOS')}
                        disabled={nivelSel===undefined}
                        style={{...g.btnPrimary,width:'100%',justifyContent:'center',
                            opacity:nivelSel!==undefined?1:0.4}}>
