    ...
    useEffect(() => {
        ...
        const handleMove = (e) => {
            ...
            if (draggingCenter.current && canvasRef.current) {
                ...
                setPos({
                    x: Math.max(0, Math.min(canvasDim.w - rulerCmLength * SCALE, clientX - rect.left - offset.current.x)),
                    y: Math.max(0, Math.min(canvasDim.h - 40, clientY - rect.top - offset.current.y)),
                });
...
const ShapeRenderer = ({ shape, params, unit, hideText, canvasDim, rulerMode }) => {
    ...
    if (!rulerMode) {
        let maxVal = 1;
        for (let k in params) if (typeof params[k] === 'number' && params[k] > maxVal) maxVal = params[k];
        activeScale = 130 / maxVal;
    }
