...
// --- IMPORTACIÓN DE AUDIOS (Igual que la versión pro) ---
...
const STYLES = `
  ...
  /* NEON ERROR MODAL COMPACTO */
  .modal-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.9); z-index: 9999; display: flex; justify-content: center; align-items: center; backdrop-filter: blur(5px); }
  .neon-card { background: #1a1a1a; padding: 25px; border-radius: 15px; border: 3px solid #ff003c; box-shadow: 0 0 20px #ff003c, inset 0 0 10px #ff003c; text-align: center; color: white; width: 90%; max-width: 400px; animation: popIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .neon-title { font-size: 1.8rem; color: #ff003c; text-shadow: 0 0 10px #ff003c; margin-bottom: 10px; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; }
  .neon-answer-label { font-size: 1rem; color: #ccc; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
  .neon-answer { font-size: 1.5rem; font-weight: bold; color: white; margin-bottom: 20px; text-shadow: 0 0 10px rgba(255, 255, 255, 0.8); line-height: 1.2; background: rgba(255, 0, 60, 0.1); padding: 10px 15px; border-radius: 8px; border: 1px solid #ff003c; width: 100%; word-wrap: break-word; box-sizing: border-box; }
  .neon-btn { background: transparent; border: 2px solid #ff003c; color: #ff003c; padding: 10px 30px; font-size: 1.1rem; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; cursor: pointer; transition: 0.3s; box-shadow: 0 0 10px #ff003c; border-radius: 50px; }
  .neon-btn:hover { background: #ff003c; color: white; box-shadow: 0 0 30px #ff003c; transform: scale(1.05); }
...
export default function PasapalabraGuest({ recurso, onBack, onLoginRequest }) {
    ...
    const iniciar = (duelo, hoja) => {
        ...
        if (duelo) {
            ...
            setJugadores([
                { nombre: 'Invitado', color: '#273c75', rosco: rosco1, aciertos: 0, fallos: 0, indice: 0, tiempo, terminado: false }
            ]);
        }
        setTurno(0);
        setFase('JUEGO');
    ...
    return (
        <div className="game-wrapper">
            ...
            {fase === 'SETUP' && !verRanking && (
                <PantallaSetup recurso={recurso} onStart={iniciar} onRanking={() => setVerRanking(true)} onExit={onBack} />
            )}

            {verRanking && (
                <PantallaRanking recurso={recurso} onBack={() => setVerRanking(false)} />
            )}
...
const PantallaRanking = ({ recurso, onBack }) => {
    const [hoja, setHoja] = useState('General');
...
// --- PANTALLA FIN (VERSIÓN MARKETING) ---
const PantallaFin = ({ jugadores, modoDuelo, playSound, onLoginRequest, onExit }) => {
    ...
    return (
        <div className="card-menu">
            ...
            {!modoDuelo && (
                <div style={{ margin: '20px 0', padding: '15px', background: 'rgba(255,255,255,0.1)', borderRadius: '15px' }}>
                    <p style={{ color: '#eee', lineHeight: '1.5' }}>
                        Regístrate para <b>guardar tus récords</b> y jugar a muchos más juegos.
                    </p>
            ...
            <button className="btn-back" onClick={onExit}>
                {modoDuelo ? 'Salir' : 'Salir al Inicio'}
            </button>
        </div>
    );
};