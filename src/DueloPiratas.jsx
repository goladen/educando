...
const FLAME_FRAMES = 210; // ~3.5 s at 60 fps
...
function genOpciones(correcta, spread) {
  const set = new Set([correcta]);
  const lo = Math.max(0, correcta - spread);
  const hi = correcta + spread + 1;
  let g = 0;
  while (set.size < 3 && g++ < 50) set.add(rnd(lo, hi));
...
const GENERATORS = {
  conteo: () => {
    const n = rnd(2, 16);
    const emojis = ['⚓','🏴‍☠️','💀','🦜','🗺️','🔱','⚔️','💎'];
    const ico = emojis[rnd(0, emojis.length - 1)];
    return { display: Array(n).fill(ico), respuesta: n, opciones: genOpciones(n, 4), tipo: 'conteo' };
  },
  sumasF:   () => { const a=rnd(1,20),b=rnd(1,20);   return { display:`${a} + ${b}`,   respuesta:a+b, opciones:genOpciones(a+b,6)  }; },
  sumasL:   () => { const a=rnd(20,89),b=rnd(11,79);  return { display:`${a} + ${b}`,   respuesta:a+b, opciones:genOpciones(a+b,12) }; },
  restas:   () => { const b=rnd(1,18),a=rnd(b+1,30);  return { display:`${a} − ${b}`,   respuesta:a-b, opciones:genOpciones(a-b,5)  }; },
  restasL:  () => { const b=rnd(12,49),a=rnd(b+10,99);return { display:`${a} − ${b}`,   respuesta:a-b, opciones:genOpciones(a-b,10) }; },
  multi5:   () => { const a=rnd(2,9),b=rnd(1,5);      return { display:`${a} × ${b}`,   respuesta:a*b, opciones:genOpciones(a*b,7)  }; },
  multi10:  () => { const a=rnd(2,10),b=rnd(1,10);    return { display:`${a} × ${b}`,   respuesta:a*b, opciones:genOpciones(a*b,12) }; },
  division: () => { const b=rnd(2,9),q=rnd(1,10);     return { display:`${b*q} ÷ ${b}`, respuesta:q,   opciones:genOpciones(q,4)    }; },
  raices:   () => { const r=rnd(1,12),n=r*r;           return { display:`√${n}`,          respuesta:r,   opciones:genOpciones(r,4)    }; },
};

const DIFICULTADES = [
  { id:'conteo',   label:'Conteo',             emoji:'⚓',  desc:'Cuenta objetos piratas' },
  { id:'sumasF',   label:'Sumas Fáciles',       emoji:'➕',  desc:'Sin llevadas, hasta 40' },
  { id:'sumasL',   label:'Sumas con Llevadas',  emoji:'➕',  desc:'Con llevadas, hasta 168' },
  { id:'restas',   label:'Restas',              emoji:'➖',  desc:'Restas simples' },
  { id:'restasL',  label:'Restas con Llevadas', emoji:'➖',  desc:'Restas difíciles' },
  { id:'multi5',   label:'Multiplicación ×5',   emoji:'✖️', desc:'Tablas del 1 al 5' },
  { id:'multi10',  label:'Multiplicación ×10',  emoji:'✖️', desc:'Tablas del 1 al 10' },
  { id:'division', label:'Divisiones',          emoji:'➗',  desc:'Divisiones exactas' },
  { id:'raices',   label:'Raíces Cuadradas',    emoji:'√',   desc:'Raíces perfectas hasta √144' },
];

// ─── FIREBASE ────────────────────────────────────────────────────────────────
async function guardarPartida(data) {
  try { await addDoc(collection(db,'duelo_piratas'),{...data,timestamp:serverTimestamp()}); }
  catch(e){ console.error('[DueloPiratas]',e); }
}

// ─── CSS ANIMATIONS ──────────────────────────────────────────────────────────
const CSS_ANIM = `
  @keyframes dp-hitText {
  ...
  @keyframes dp-rock {
    ...
    12%  { transform: rotate(-11deg)  translateY(-7px); }
    28%  { transform: rotate(9deg)    translateY(-4px); }
    46%  { transform: rotate(-6deg)   translateY(-2px); }
    62%  { transform: rotate(5deg)    translateY(-1px); }
    78%  { transform: rotate(-2.5deg) translateY(0px); }
    90%  { transform: rotate(1.5deg)  translateY(0px); }
    100% { transform: rotate(-1.8deg) translateY(0px); }
  }
...
function MenuPantalla({ onStart, onExit }) {
  return (
    <div style={{
      ...
      background:'linear-gradient(180deg,#07111f 0%,#0e2a44 45%,#0a4060 100%)',
      ...
      <div style={{textAlign:'center',marginBottom:28}}>
        ...
        <h1 style={{margin:0,color:'#f9c74f',fontSize:'clamp(2rem,5vw,3rem)',fontFamily:'Georgia,serif',textShadow:'0 0 24px rgba(249,199,79,.55), 2px 2px 0 #7a4f00'}}>
          Duelo de Piratas
        </h1>
        <p style={{color:'#94d2bd',margin:'8px 0 0',fontSize:'0.95rem'}}>Elige la operación y ¡a batirse a cañonazos!</p>
      </div>
      <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.14)',borderRadius:14,padding:'14px 20px',marginBottom:28,maxWidth:520,width:'100%'}}>
        <p style={{color:'#ddd',margin:0,fontSize:'0.85rem',lineHeight:1.8,textAlign:'center'}}>
          🎯 <strong style={{color:'#f9c74f'}}>Sincroniza el cañón:</strong> responde cuando la mira esté en zona <span style={{color:'#00e676'}}>VERDE</span><br/>
          💨 Correcto fuera de zona → la bala cae al mar<br/>
          ❌ Respuesta incorrecta → penalización de {(COOLDOWN_MS/1000).toFixed(1)}s<br/>
          💀 Primer barco a 0 vidas pierde · ¡2 jugadores local!
        </p>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(148px,1fr))',gap:12,width:'100%',maxWidth:640}}>
        {DIFICULTADES.map(d => (
          <button key={d.id} onClick={() => onStart(d.id)}
            style={{background:'rgba(255,255,255,0.07)',border:'2px solid rgba(249,199,79,0.35)',borderRadius:14,padding:'16px 10px',cursor:'pointer',color:'white',display:'flex',flexDirection:'column',alignItems:'center',gap:6,transition:'all 0.18s'}}
            onMouseEnter={e=>{e.currentTarget.style.background='rgba(249,199,79,0.14)';e.currentTarget.style.borderColor='#f9c74f';e.currentTarget.style.transform='translateY(-4px)';}}
            onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.07)';e.currentTarget.style.borderColor='rgba(249,199,79,0.35)';e.currentTarget.style.transform='translateY(0)';}}
          >
            <span style={{fontSize:'1.9rem'}}>{d.emoji}</span>
            <span style={{fontWeight:800,fontSize:'0.88rem',textAlign:'center'}}>{d.label}</span>
            <span style={{color:'#94d2bd',fontSize:'0.73rem',textAlign:'center'}}>{d.desc}</span>
          </button>
        ))}
      </div>
      <button onClick={onExit} style={{marginTop:28,background:'transparent',border:'1px solid rgba(255,255,255,0.25)',color:'#999',borderRadius:8,padding:'8px 28px',cursor:'pointer',fontSize:'0.9rem'}}>← Volver</button>
    </div>
  );
}

// ─── GAME OVER ───────────────────────────────────────────────────────────────
function GameOverPantalla({ ganador, stats, dificultad, onReintentar, onMenu, onExit }) {
  const difLabel = DIFICULTADES.find(d => d.id === dificultad)?.label || dificultad;
  return (
    <div style={{position:'fixed',inset:0,zIndex:9999,background:'radial-gradient(circle at 50% 35%,#1a3a5c 0%,#07111f 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',color:'white',padding:24,overflowY:'auto'}}>
      ...
      <p style={{color:'#94d2bd',marginBottom:28,fontSize:'1.05rem'}}>{NOMBRES[ganador]} gana · <span style={{opacity:0.7}}>{difLabel}</span></p>
      ...
      <div style={{display:'flex',gap:12,flexWrap:'wrap',justifyContent:'center'}}>
        ...
        <button onClick={onMenu} style={{background:'rgba(255,255,255,0.1)',color:'#fff',border:'1px solid rgba(255,255,255,0.28)',borderRadius:10,padding:'12px 26px',fontWeight:700,fontSize:'1rem',cursor:'pointer'}}>📋 Menú</button>
...
// ─── SHIP COLUMN ─────────────────────────────────────────────────────────────
// side: 'left'(Blue) | 'right'(Red)
// Renders: badge → hearts → spacer → ship+cannon at waterline
function ShipColumn({ playerId, side, vidas, shipRef, cañonRef, muzzleRef }) {
  ...
  // Cannon mount wrapper: positions the pivot (mount) at the stern
  const mountWrapStyle = isRight
    ? { position:'absolute', left:'14%', top:'60%' }
    : { position:'absolute', right:'14%', top:'60%' };

  const cannonBase = {
    position:'absolute',
    width:'clamp(38px,5.5vw,56px)', height:13,
    background:'linear-gradient(to bottom,#6a6a6a,#1c1c1c)',
    boxShadow:'0 2px 6px rgba(0,0,0,0.7)',
    transformOrigin: isRight ? 'right center' : 'left center',
    borderRadius: isRight ? '5px 2px 2px 5px' : '2px 5px 5px 2px',
  };

  // For Red (isRight): cannon extends LEFT, right edge = mount → right:0
  // For Blue (!isRight): cannon extends RIGHT, left edge = mount → left:0
  const cannonInnerStyle = isRight
    ? { right:0, top:'-6px', ...cannonBase }
    : { left:0,  top:'-6px', ...cannonBase };

  // Muzzle marker: child of rotating cannon → getBoundingClientRect() gives real tip position
  // Red muzzle at LEFT end of cannon (tip); Blue muzzle at RIGHT end
  const muzzleStyle = isRight
    ? { position:'absolute', left:0,  top:'50%', transform:'translateY(-50%)', width:2, height:2, pointerEvents:'none' }
    : { position:'absolute', right:0, top:'50%', transform:'translateY(-50%)', width:2, height:2, pointerEvents:'none' };
  ...
  return (
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',padding:'10px clamp(8px,2vw,20px) 0'}}>
      ...
      {/* 3. Spacer → pushes ship to bottom / waterline */}
      <div style={{flex:1}}/>

      {/* 4. Ship + cannon at waterline */}
      <div ref={shipRef} className="dp-sway" style={{position:'relative',display:'inline-block',marginBottom:'-8px'}}>
        <img src={img} alt={NOMBRES[playerId]} style={{
          width:'clamp(120px,17vw,210px)', height:'auto', display:'block',
          filter: vidas<=3 ? 'drop-shadow(0 0 14px rgba(255,50,50,.95))' : 'drop-shadow(0 6px 10px rgba(0,0,0,0.6))',
          transition:'filter 0.35s',
        }}/>

        {/* Cannon mount at stern */}
        <div style={mountWrapStyle}>
          <div ref={cañonRef} style={cannonInnerStyle}>
            {/* Muzzle position marker (rotates with cannon) */}
...
function ControlPanel({ playerId, pregunta, cooldown, aimRef, onRespuesta, hitStart, hitWidth }) {
  const color = COLORES[playerId];
  return (
    <div style={{flex:1,position:'relative',overflow:'hidden',background:`linear-gradient(135deg,${color}1a 0%,${color}09 100%)`,border:`2px solid ${color}55`,borderRadius:14,padding:'10px 12px'}}>
      {cooldown && (
      ...
      <div style={{marginBottom:8}}>
        <div style={{fontSize:'0.6rem',color:'#888',textAlign:'center',marginBottom:3}}>Zona de disparo</div>
        <div style={{position:'relative',height:18,background:'rgba(0,0,0,.45)',borderRadius:9,overflow:'hidden'}}>
          ...
          <div style={{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%,-50%)',fontSize:'0.55rem',opacity:0.55,pointerEvents:'none'}}>🎯</div>
          <div ref={aimRef} style={{position:'absolute',top:1,bottom:1,width:4,borderRadius:2,background:'#ff5252',transform:'translateX(-50%)'}}/>
        </div>
      </div>
      {/* Question */}
      <div style={{textAlign:'center',marginBottom:10,minHeight:50,display:'flex',alignItems:'center',justifyContent:'center'}}>
        {pregunta?.tipo === 'conteo' ? (
          <div style={{display:'flex',flexWrap:'wrap',gap:3,maxWidth:200,justifyContent:'center',fontSize:'clamp(0.85rem,1.3vw,1.1rem)'}}>
            {(pregunta.display||[]).map((ico,i) => <span key={i}>{ico}</span>)}
          </div>
        ) : (
          <div style={{fontSize:'clamp(1.5rem,3.5vw,2.2rem)',fontWeight:900,color:'white',fontFamily:'monospace'}}>
            {pregunta?.display ?? '...'}
          </div>
        )}
      </div>
      {/* Buttons */}
      <div style={{display:'flex',gap:'clamp(6px,1.5vw,14px)',justifyContent:'center'}}>
        {pregunta?.opciones?.map((op,i) => (
          <button key={i} onClick={() => onRespuesta(op)}
            style={{width:'clamp(46px,7.5vw,68px)',height:'clamp(46px,7.5vw,68px)',borderRadius:'50%',border:`3px solid ${color}`,background:'rgba(255,255,255,.94)',color,fontWeight:900,fontSize:'clamp(0.85rem,1.5vw,1.2rem)',cursor:'pointer',boxShadow:`0 4px 14px ${color}55`,transition:'transform 0.1s'}}
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function DueloPiratas({ onExit }) {
  const [fase, setFase]         = useState('menu');
  const [dificultad, setDif]    = useState('sumasF');
  ...
  const muzzleEls      = useRef([null, null]);   // tip of each cannon (rotates with it)
  const aimEls         = useRef([null, null]);
  const canvasRef      = useRef(null);
  const balasRef       = useRef([]);             // {id,x,y,tx,ty,prog,pl,hit,arrived}
  const balaId         = useRef(0);
  const flameEmitters  = useRef([]);             // {cx,cy,framesLeft}
  const flameParticles = useRef([]);             // {cx,cy,vx,vy,r,age,maxAge,hue}
  const vivoRef        = useRef(false);
  const vidasRef       = useRef([VIDAS_MAX, VIDAS_MAX]);
  const cooldownRef    = useRef([false, false]);
  const dificultadRef  = useRef('sumasF');
  ...
  useEffect(() => {
    ...
    // Size canvas now that it's in the DOM (resize effect ran during menu phase when canvas wasn't mounted yet)
    const c = canvasRef.current;
    ...
    // Sound helper usable inside RAF (closes over sfxRef)
    const playSfx = (key) => {
      try {
    ...
    // Spawn a sustained flame on a ship position
    const addFlame = (enemigo) => {
      const el = shipEls.current[enemigo];
      if (!el) return;
      const r = el.getBoundingClientRect();
      flameEmitters.current.push({
        cx: r.left + r.width * 0.5,
        cy: r.top  + r.height * 0.35,
        framesLeft: FLAME_FRAMES,
      });
    };
    ...
    const loop = () => {
      ...
      for (let p = 0; p < 2; p++) {
        ...
        const cEl = cañonEls.current[p];
        if (cEl) cEl.style.transform = `rotate(${-ang}deg)`;
      ...
      // ── Cannonballs ────────────────────────────────────────────────────────
      if (ctx) {
        const ARC = 140; // parabolic height in px

        const pos = (b, t) => ({
          x: b.x + (b.tx - b.x) * t,
          y: b.y + (b.ty - b.y) * t - Math.sin(Math.max(0, Math.min(1, t)) * Math.PI) * ARC,
        });
        ...
        for (const b of balasRef.current) {
          ...
          // Smoke trail (5 fading puffs)
          for (let j = 1; j <= 5; j++) {
            const tp = pos(b, Math.max(0, t - j * 0.04));
            const alpha  = (1 - j * 0.18) * 0.55;
            const radius = Math.max(1, 9 - j * 1.5);
            ctx.beginPath();
            ctx.arc(tp.x, tp.y, radius, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(190,155,100,${alpha})`;
            ctx.fill();
          }
          ...
          ctx.fillStyle = b.pl === 0 ? 'rgba(255,110,0,0.35)' : 'rgba(60,140,255,0.35)';
          ctx.fill();

          // Iron cannonball – radial gradient
          ...
          ctx.fillStyle = grad;
          ctx.shadowBlur  = 22;
          ctx.shadowColor = b.pl === 0 ? '#ff8800' : '#3388ff';
          ctx.fill();
          ctx.shadowBlur = 0;

          // Glint highlight
          ctx.beginPath();
          ctx.arc(cur.x - 4, cur.y - 4, 3.5, 0, Math.PI * 2);
          ctx.fillStyle = 'rgba(255,255,255,0.5)';
          ctx.fill();
        }
      }

      // ── Flame emitters → spawn particles ──────────────────────────────────
      for (const em of flameEmitters.current) {
        em.framesLeft--;
        if (em.framesLeft % 2 === 0) {         // spawn every 2 frames
          for (let k = 0; k < 4; k++) {
            flameParticles.current.push({
              cx:     em.cx + (Math.random() - 0.5) * 50,
              cy:     em.cy + (Math.random() - 0.5) * 25,
              vx:     (Math.random() - 0.5) * 2.2,
              vy:     -Math.random() * 3.5 - 0.8,
              r:      Math.random() * 13 + 6,
              age:    0,
              maxAge: 28 + Math.random() * 22,
              hue:    Math.random() * 38,       // 0=red … 38=orange
            });
          }
        }
      }
      ...
      if (ctx) {
        ...
        for (const f of flameParticles.current) {
          ...
          f.cx += f.vx;
          f.cy += f.vy;
          f.vy -= 0.09;          // accelerate upward
          const prog   = f.age / f.maxAge;
          const alpha  = (1 - prog) * 0.92;
          const radius = Math.max(0.5, f.r * (1 - prog * 0.65));
          const light  = 48 + prog * 28;
          ctx.beginPath();
          ctx.arc(f.cx, f.cy, radius, 0, Math.PI * 2);
          ctx.fillStyle  = `hsla(${f.hue + 5}, 100%, ${light}%, ${alpha})`;
          ctx.shadowBlur  = 7;
  ...
  // ── Sound helper (for event handlers) ─────────────────────────────────────
  const playSfx = (key) => {
    try {
      const a = sfxRef.current?.[key];
      if (!a) return;
  ...
  const triggerShake = (playerHit) => {
    const el = shipEls.current[playerHit];
    if (!el) return;
    el.classList.remove('dp-sway', 'dp-ship-rock');
    void el.offsetWidth;
  ...
  const showMsg = (text, color, playerId) => {
    ...
    const yPct = 18 + Math.random()*14;
    setMensajes(prev => [...prev, {id, text, color, x: xPct, y: yPct}]);
    setTimeout(() => setMensajes(prev => prev.filter(m => m.id !== id)), 950);
  };

  // ── Fire cannonball (origin = actual muzzle position from DOM) ─────────────
  const disparar = (playerId, hit) => {
    // Get muzzle start position from the rotating cannon tip
    let startX, startY;
    ...
    if (muzzleEl) {
      ...
      startX = playerId === 0 ? W * 0.78 : W * 0.22;
      startY = H * 0.50;
    }
    ...
    if (hit) {
      ...
      const shipEl  = shipEls.current[enemigo];
      if (shipEl) {
        ...
        const W = window.innerWidth, H = window.innerHeight;
        endX = playerId === 0 ? W * 0.22 : W * 0.78;
        endY = H * 0.50;
      }
    } else {
      // Miss: fly in cannon's current direction past the centre
      const ang = angulosRef.current[playerId];
      const dir = playerId === 0 ? -1 : 1;   // Red fires left, Blue fires right
      const W = window.innerWidth;
      endX = startX + dir * W * 0.30;
      endY = startY - ang * 5;
    }

    playSfx('disparo');
    balasRef.current.push({
      id: balaId.current++, x: startX, y: startY,
      tx: endX, ty: endY, prog: 0, pl: playerId, hit, arrived: false,
    });
  };
  ...
  const terminarJuego = (ganadorId, finalStats) => {
    ...
    setFase('gameover');
    guardarPartida({
      ganador: NOMBRES[ganadorId], dificultad: dificultadRef.current,
      jugadores: [
        {color:'Rojo', ...finalStats[0]},
        {color:'Azul', ...finalStats[1]},
      ],
    });
  };
  ...
  const handleRespuesta = (playerId, opcion) => {
    ...
    if (esCorrecta && enZona) {
      playSfx('correcto');
      disparar(playerId, true);
      showMsg('💥 ¡IMPACTO!', '#00e676', playerId);
      newStats[playerId].aciertos++;
      newStats[playerId].balasDisparadas++;
      newStats[playerId].balasImpacto++;
      const enemigo  = 1 - playerId;
      const newVidas = [...vidasRef.current];
      newVidas[enemigo] = Math.max(0, newVidas[enemigo] - 1);
      vidasRef.current  = newVidas;
      setVidas([...newVidas]);
      triggerShake(enemigo);
      if (newVidas[enemigo] <= 0) {
        statsRef.current = newStats; setStats([...newStats]);
        terminarJuego(playerId, newStats);
        return;
      }
    } else if (esCorrecta && !enZona) {
      playSfx('correcto');
      disparar(playerId, false);
      showMsg('💨 ¡Fuera!', '#ffb347', playerId);
      newStats[playerId].aciertos++;
      newStats[playerId].balasDisparadas++;
    } else {
      playSfx('fallo');
      showMsg('✗ Error', '#ff5252', playerId);
    ...
    setStats([...newStats]);
    const newQ = GENERATORS[dificultadRef.current]();
    preguntasRef.current = [...preguntasRef.current];
    preguntasRef.current[playerId] = newQ;
    setPregs([...preguntasRef.current]);
  };

  // ── Start game ─────────────────────────────────────────────────────────────
  const iniciarJuego = (dif) => {
    dificultadRef.current = dif;
    setDif(dif);
    const gen = GENERATORS[dif];
    const q0 = gen(), q1 = gen();
    preguntasRef.current = [q0, q1];
    setPregs([q0, q1]);
    vidasRef.current = [VIDAS_MAX, VIDAS_MAX];
    setVidas([VIDAS_MAX, VIDAS_MAX]);
    cooldownRef.current = [false, false];
    ...
    setMensajes([]);
    balasRef.current   = [];
    flameEmitters.current  = [];
    flameParticles.current = [];
    angulosRef.current = [0, 20];
    dirRef.current     = [1, -1];
    setFase('game');
  };
  ...
  if (fase === 'gameover') return (
    <GameOverPantalla
      ganador={ganador} stats={stats} dificultad={dificultad}
      onReintentar={() => iniciarJuego(dificultad)}
      onMenu={() => setFase('menu')} onExit={onExit}
    />
  );

  const difLabel = DIFICULTADES.find(d => d.id === dificultad)?.label || '';
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:9999,overflow:'hidden',userSelect:'none',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{position:'relative',zIndex:10,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'5px 14px',background:'rgba(0,0,0,.7)',borderBottom:'1px solid rgba(255,255,255,.08)'}}>
        <span style={{color:'#94d2bd',fontSize:'0.78rem'}}>⚓ {difLabel}</span>
      ...
      {/* Ships arena: Blue LEFT | VS | Red RIGHT */}
      <div style={{position:'relative',zIndex:2,flex:1,display:'flex',minHeight:0,overflow:'visible'}}>
        ...
        {/* Blue (p=1) – LEFT */}
        <ShipColumn
          playerId={1} side="left" vidas={vidas[1]}
        ...
        {/* Red (p=0) – RIGHT */}
        <ShipColumn
          playerId={0} side="right" vidas={vidas[0]}
      ...
      {/* Control panels: Blue LEFT | Red RIGHT (matches ship layout) */}
