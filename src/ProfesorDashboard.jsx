...
export default function ProfesorDashboard({ usuario, googleToken }) {
    ...
    const [vista, setVista] = useState('MIS_RECURSOS');
                    ...
                    <div key={r.id || i} style={{ background: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', borderLeft: `6px solid ${TIPOS_JUEGOS[juegoSeleccionado].color}`, position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', gap: '5px', marginTop: '10px' }}>
                            {juegoSeleccionado === 'QUESTION_SENDER' ? (
                                <><button onClick={() => abrirEdicion(r)} style={btnStyle('#E3F2FD', '#1565C0')}><Pencil size={18} /></button><button onClick={() => setModalCopiarApp(r)} style={btnStyle('#E8F5E9', '#2E7D32')}><Send size={18} /></button><button onClick={() => eliminarRecurso(r.id)} style={btnStyle('#FFEBEE', '#C62828')}><Trash2 size={18} /></button></>
                            ) : (
                                    vista === 'MIS_RECURSOS' ? (
                                        <>
                                            ...
                                            <button onClick={() => eliminarRecurso(r.id)} style={btnStyle('#FFEBEE', '#C62828')}><Trash2 size={18} /></button>
                                        </>
                                    ) : (
                                            <><button onClick={() => probarJuego(r)} style={{ ...btnStyle('#E1BEE7', '#8E24AA'), flex: 2 }}>Probar</button><button onClick={() => setRecursoInspeccionando(r)} style={btnStyle('#eee', '#333')}><Eye size={18} /></button><button onClick={() => copiarRecurso(r)} style={{ ...btnStyle('#27ae60', 'white'), flex: 2 }}>Copiar</button></>
                                        )
                                )}
                        </div>
                    </div>
