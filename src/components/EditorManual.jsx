...
import { Save, X, Trash2, FolderPlus, ArrowUp, ArrowDown, Lock, Globe } from 'lucide-react';
...
export default function EditorManual({ datos, setDatos, configJuego, onClose, onSave }) {
    ...
    const renameHoja = (idx, val) => { const n = [...datos.hojas]; n[idx].nombreHoja = val; setDatos({ ...datos, hojas: n }); };

    // Gestión Preguntas (Igual que antes)
    const addPregunta = () => {
        ...
        else p = { pregunta: '', correcta: '', incorrectas: ['', '', ''] };
        nuevas[hojaActiva].preguntas.push(p);
        setDatos({ ...datos, hojas: nuevas });
    };

    const updatePregunta = (idx, field, val) => { const nuevas = [...datos.hojas]; nuevas[hojaActiva].preguntas[idx][field] = val; setDatos({ ...datos, hojas: nuevas }); };
    const deletePregunta = (idx) => { const nuevas = [...datos.hojas]; nuevas[hojaActiva].preguntas.splice(idx, 1); setDatos({ ...datos, hojas: nuevas }); };
    const moverPregunta = (idx, dir) => {
        const nuevas = [...datos.hojas]; const list = nuevas[hojaActiva].preguntas;
        if (dir === -1 && idx > 0) [list[idx], list[idx - 1]] = [list[idx - 1], list[idx]];
        if (dir === 1 && idx < list.length - 1) [list[idx], list[idx + 1]] = [list[idx + 1], list[idx]];
        setDatos({ ...datos, hojas: nuevas });
    };

    // Render Campos (Igual que antes)
    const renderCampos = (p, i) => {
        if (configJuego.id === 'PASAPALABRA') {
            return (<div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}><input style={{ width: '40px', textAlign: 'center' }} placeholder="A" value={p.letra} onChange={e => updatePregunta(i, 'letra', e.target.value.toUpperCase())} className="inp" /><input style={{ flex: 2 }} placeholder="Pregunta" value={p.pregunta} onChange={e => updatePregunta(i, 'pregunta', e.target.value)} className="inp" /><input style={{ flex: 1 }} placeholder="Respuesta" value={p.respuesta} onChange={e => updatePregunta(i, 'respuesta', e.target.value)} className="inp" /></div>);
        } else if (configJuego.id === 'APAREJADOS') {
            return (<div style={{ display: 'flex', gap: '10px' }}><input style={{ flex: 1 }} placeholder="Término A" value={p.terminoA} onChange={e => updatePregunta(i, 'terminoA', e.target.value)} className="inp" /><span style={{ paddingTop: '5px' }}>↔️</span><input style={{ flex: 1 }} placeholder="Término B" value={p.terminoB} onChange={e => updatePregunta(i, 'terminoB', e.target.value)} className="inp" /></div>);
        } else {
            return (<div style={{ display: 'flex', flexDirection: 'column', gap: '5px', width: '100%' }}><input placeholder="Pregunta..." value={p.pregunta} onChange={e => updatePregunta(i, 'pregunta', e.target.value)} className="inp" style={{ fontWeight: 'bold' }} /><div style={{ display: 'flex', gap: '5px' }}><input style={{ flex: 1, borderColor: '#2ecc71' }} placeholder="✅ Correcta" value={p.correcta} onChange={e => updatePregunta(i, 'correcta', e.target.value)} className="inp" /><input style={{ flex: 2, borderColor: '#e74c3c' }} placeholder="❌ Incorrectas (sep. por comas)" value={Array.isArray(p.incorrectas) ? p.incorrectas.join(',') : p.incorrectas} onChange={e => updatePregunta(i, 'incorrectas', e.target.value.split(','))} className="inp" /></div></div>);
        }
    };
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', zIndex: 2000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <div style={{ background: 'white', width: '95%', height: '95%', borderRadius: '15px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 0 20px rgba(0,0,0,0.5)' }}>
                ...
                <div style={{ padding: '15px', background: '#f5f5f5', borderBottom: '1px solid #ddd', display: 'flex', gap: '20px', alignItems: 'center' }}>
                    ...
                    {/* CHECKBOX PRIVACIDAD */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'white', padding: '5px 10px', borderRadius: '20px', border: '1px solid #ddd' }}>
                        <label style={{ fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                            {datos.isPrivate ? <Lock size={16} color="#e74c3c" /> : <Globe size={16} color="#2ecc71" />}
                            <input
                                type="checkbox"
                                checked={datos.isPrivate || false}
                                onChange={e => setDatos({ ...datos, isPrivate: e.target.checked })}
                                style={{ cursor: 'pointer' }}
                            />
                            {datos.isPrivate ? "Privado (No compartir)" : "Público (Biblioteca)"}
                        </label>
                    </div>
                ...
                <div style={{ padding: '15px', background: '#fff', borderBottom: '1px solid #eee', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px' }}>
                    {configJuego.camposConfig.map(c => (
                        <div key={c.key}><label style={{ fontSize: '11px', color: '#666' }}>{c.label}</label><input type={c.type} value={datos.config[c.key]} onChange={e => updateConfig(c.key, e.target.value)} style={inputStyleSmall} /></div>
                    ))}
                    ...
                    <div><label style={{ fontSize: '11px', color: '#666' }}>Región</label><input value={datos.region} onChange={e => setDatos({ ...datos, region: e.target.value })} style={inputStyleSmall} /></div>
                ...
                {/* TABS Y PREGUNTAS (Sin cambios) */}
                <div style={{ background: '#e0e0e0', padding: '10px 10px 0 10px', display: 'flex', gap: '5px', overflowX: 'auto' }}>
                    {datos.hojas.map((h, i) => (
                        <div key={i} onClick={() => setHojaActiva(i)} style={{ padding: '8px 15px', background: i === hojaActiva ? 'white' : '#ccc', borderRadius: '8px 8px 0 0', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px', minWidth: '100px' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px', background: '#f9f9f9' }}>
                    {datos.hojas[hojaActiva]?.preguntas.length === 0 ? (
                        <div style={{ textAlign: 'center', color: '#888', marginTop: '50px' }}><p>Hoja vacía.</p><button onClick={addPregunta} style={{ padding: '10px 20px', background: '#2196F3', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>+ Añadir Pregunta</button></div>
                    ) : (
                            datos.hojas[hojaActiva]?.preguntas.map((p, i) => (
                                <div key={i} style={{ background: 'white', padding: '15px', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', marginBottom: '10px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                        <button onClick={() => moverPregunta(i, -1)} disabled={i === 0} style={arrowBtn}><ArrowUp size={12} /></button><span style={{ fontSize: '12px', fontWeight: 'bold', textAlign: 'center', color: '#888' }}>{i + 1}</span><button onClick={() => moverPregunta(i, 1)} disabled={i === datos.hojas[hojaActiva].preguntas.length - 1} style={arrowBtn}><ArrowDown size={12} /></button>
                                    </div>
                    ...
                    {datos.hojas[hojaActiva]?.preguntas.length > 0 && <button onClick={addPregunta} style={{ width: '100%', padding: '15px', border: '2px dashed #ccc', background: 'transparent', borderRadius: '10px', color: '#666', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px' }}>+ AÑADIR OTRA PREGUNTA</button>}
                </div>
            </div>
        </div>
    );
