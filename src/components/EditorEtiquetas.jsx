                    ...
                    <div style={st.modalOverlay}>
                        <div style={st.modal}>
                            ...
                            <div style={st.modalBody}>
                                ...
                                <div style={{ marginBottom: 12 }}>
                                    ...
                                    <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid #ddd', marginBottom: 8 }}>
                                        {[['imagen', '🖼️ Imagen'], ['video', '▶️ Vídeo']].map(([tipo, lbl]) => (
                                            <button key={tipo} onClick={() => { setMediaTipoPres(tipo); setModoBuscadorPres(false); setSearchResultsPres([]); }} style={{ flex: 1, padding: '7px 0', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem', background: mediaTipoPres === tipo ? '#16a085' : '#f5f5f5', color: mediaTipoPres === tipo ? 'white' : '#555' }}>{lbl}</button>
                                        ))}
                                    </div>
                                    {mediaTipoPres === 'imagen' && (<>
                                        ...
                                        <button onClick={() => setModoBuscadorPres(p => !p)} style={{ ...st.input, background: '#e0f2f1', color: '#00695c', fontWeight: 'bold', border: '1px dashed #26a69a', cursor: 'pointer', textAlign: 'center', marginBottom: 6 }}>🔍 {modoBuscadorPres ? 'Cerrar buscador' : 'Buscar imagen'}</button>
                                        {modoBuscadorPres && (<div style={{ background: '#f5f5f5', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                                            ...
                                            {searchResultsPres.length > 0 && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(72px,1fr))', gap: 5, maxHeight: 170, overflowY: 'auto' }}>{searchResultsPres.map((url, i) => <img key={i} src={url} alt="" onClick={() => setPresentacionImg(url)} onError={e => { e.target.style.display = 'none'; }} style={{ width: '100%', height: 60, objectFit: 'cover', borderRadius: 5, cursor: 'pointer', border: '2px solid transparent' }} onMouseEnter={e => { e.target.style.border = '2px solid #16a085'; }} onMouseLeave={e => { e.target.style.border = '2px solid transparent'; }} />)}</div>)}
                                        </div>)}
                                    </>)}
                                    {mediaTipoPres === 'video' && (<>
                                ...
                                <div style={{ marginBottom: 12 }}><label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', color: '#555', marginBottom: 4 }}>Descripción</label><textarea value={datos.presentacion?.descripcion || ''} onChange={e => setDatos(p => ({ ...p, presentacion: { ...(p.presentacion || {}), descripcion: e.target.value } }))} style={{ ...st.input, resize: 'vertical', minHeight: 58, fontFamily: 'inherit' }} placeholder="Texto introductorio..." /></div>
