        ...
        <div style={{ position:'absolute', left:mesa.x, top:mesa.y, width:w, borderRadius:10, border:`2px solid ${col}`, background:'white', boxShadow:'0 2px 8px rgba(0,0,0,0.10)', overflow:'hidden', userSelect:'none', zIndex:1 }}>
            ...
            <div style={{ display:'flex', gap:PLANO_SG, padding:`4px ${PLANO_PAD/2}px` }}>
                {mesa.asientos.map((s, idx) => {
                    ...
                    const border = isSel ? '2px solid #e74c3c' : isTarget ? '2px dashed #1565C0' : s.alumno ? '1.5px solid #a5d6a7' : '1.5px solid #f5c6c2';
                    return (
                        <div key={s.id} onClick={() => { if (!isP) onClickSeat(mesa.id, idx); }}
                            style={{ width:PLANO_SW, minHeight:42, borderRadius:6, border, background:bg, cursor:isP?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:'2px 3px', textAlign:'center', fontSize:'0.65rem', fontWeight:600, color:'#2c3e50', wordBreak:'break-word', lineHeight:1.2, transition:'all 0.1s' }}>
                            {isP
