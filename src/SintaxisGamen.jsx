        ...
        try {
            ...
            const jugadoresInforme = esLive ? datos.jugadores
                : [{ nombre:nombre.trim(), curso:curso.trim(), puntos:datos.puntos||0,
                     intentos:datos.intentos||datos.total||1,
...
function ModoLocalCA({ nivel, onBack }) {
    ...
    return(
        <div style={g.container}>
            ...
            {mostrarEnvio&&<ModalEnviarProfe datos={{intentos:1,aciertos:Object.values(results||{}).filter(r=>r.ok).length,porcentaje:pct,puntos:score,nivel:nivLabel,idioma:'CA',modalidad:'Lliure'}} onClose={()=>setMostrarEnvio(false)}/>}
        </div>
    );
}
...
function SintaxisAppCA({ onExit }) {
    ...
    if(screen==='MODOS') return(
        <div style={g.container}>
            <div style={g.header}><button onClick={()=>setScreen('NIVEL')} style={g.btnBack}><RotateCcw size={16}/> Tornar</button><span style={g.titulo}>🏴 Sintaxi Catalana</span><div style={{width:60}}/></div>
            <div style={g.centerBox}>
    ...
    return(
        <div style={g.container}>
            <div style={g.header}><button onClick={onExit} style={g.btnBack}><RotateCcw size={16}/> Sortir</button><span style={g.titulo}>🏴 Sintaxi Catalana</span><div style={{width:60}}/></div>
            <div style={g.centerBox}>
                <div style={{...g.card,maxWidth:520}}>
...
function ModoLocalFR({ nivel, onBack }) {
    ...
    const [mostrarEnvio, setMostrarEnvio] = useState(false);
    const [frasesOk, setFrasesOk] = useState(0);
    const [totalFrases, setTotalFrases] = useState(0);
    const nivLabel = NIVELES_FR.find(n=>n.id===nivel)?.label||'Tous';
    ...
    const comprobar = () => {
        ...
        if (allOk) { setStreak(s=>s+1); setFrasesOk(n=>n+1); } else setStreak(0);
                ...
                {!results ? (
                    ...
                    <>
                        ...
                        <div style={{display:'flex',gap:9,marginTop:12}}>
                            ...
                            <button onClick={()=>setMostrarEnvio(true)} style={{...g.btnPrimary,padding:'10px 16px'}}><Send size={14}/> Prof</button>
                        </div>
                    </>
                )}
            </div>
            {mostrarEnvio&&<ModalEnviarProfe datos={{intentos:1,aciertos:frasesOk>0?1:0,porcentaje:totalFrases>0?Math.round(frasesOk/totalFrases*100):0,puntos:score,nivel:nivLabel,idioma:'FR',modalidad:'Mode Libre'}} onClose={()=>setMostrarEnvio(false)}/>}
        </div>
    );
}
...
function ModoContrarrelojFR({ nivel, onBack }) {
    ...
    const comprobar = () => {
        ...
        const res = {};
        frase.tokens.forEach((t,i) => {
            const esp = t.funcion||[], ans = answers[i]||[]; tot++;
            const ok = esp.length===ans.length && esp.every(f=>ans.includes(f));
            res[i] = { ok, esp, dad:ans };
    ...
    if (fase==='FIN') {
        const frasesTotFR = frasesDone+(results?1:0);
        const pctFR = total>0 ? Math.round(aciertos/total*100) : 0;
        const nivLabelFR = NIVELES_FR.find(n=>n.id===nivel)?.label||'';
        return (
        <div style={g.container}>
            ...
            <div style={g.centerBox}><div style={{...g.card,maxWidth:420,textAlign:'center'}}>
                ...
                <p style={{color:'#666'}}>{frasesTotFR} phrases analysées · {pctFR}% acierto</p>
                <div style={{display:'flex',gap:9,marginTop:16,justifyContent:'center'}}>
                    <button onClick={()=>setMostrarEnvio(true)} style={{...g.btnPrimary,fontSize:'0.9rem'}}><Send size={14}/> Envoyer au prof</button>
                    <button onClick={onBack} style={{...g.btnGray,fontSize:'0.9rem'}}><RotateCcw size={16}/> Rejouer</button>
                </div>
            </div></div>
            {mostrarEnvio&&<ModalEnviarProfe datos={{intentos:frasesTotFR,aciertos,porcentaje:pctFR,puntos:score,nivel:nivLabelFR,idioma:'FR',modalidad:'Contre-la-montre'}} onClose={()=>setMostrarEnvio(false)}/>}
        </div>
        );
    }
    return (
    ...
    return (
        <div style={g.container}>
            ...
            {mostrarEnvio && (() => {
                ...
                const nPreg=sala.preguntas?.length||1;const datosHost={modalidad:'Online',idioma:sala.idioma||'ES',jugadores:sorted2.map(([id,j])=>({nombre:j.nombre||id,curso:'',puntos:j.puntos||0,intentos:nPreg,aciertos:Math.round(Math.round((j.puntos||0)/maxPts2*100)*nPreg/100),porcentaje:Math.round((j.puntos||0)/maxPts2*100),nivel:sala.config?.nivel||'',idioma:sala.idioma||'ES'}))};
...
function FinClientSintaxis({ sorted, myScore, myPos, nombre, uid, sala, onExit }) {
    ...
    const numPreguntas = sala?.preguntas?.length || 1;
    const datos = {
        modalidad: 'Online', idioma,
        jugadores: sorted.map(([id,j])=>({
            nombre:j.nombre||id, curso:'', puntos:j.puntos||0,
            intentos:numPreguntas,
            aciertos:Math.round(Math.round((j.puntos||0)/maxPts*100)*numPreguntas/100),
