...
export default function EditorOlympic({ datos, setDatos, onClose, onSave, usuario }) {
    ...
    // Inicialización: Asegurar tipo OLYMPIC y configuración base
    useEffect(() => {
        if (datos) {
            setDatos(prev => {
                const nuevaConfig = { ...prev.config };

                // Opcionalidad de mates
                if (nuevaConfig.usarMates === undefined) nuevaConfig.usarMates = true;

                // Valores por defecto para MathLive
                ...
                if (nuevaConfig.numPreguntas === undefined) nuevaConfig.numPreguntas = 4;

                return {
                    ...prev,
                    tipo: 'OLYMPIC', // Se guarda como OLYMPIC
                    config: nuevaConfig,
    ...
    // --- NUEVO: CARGAR LOS RECURSOS DEL PROFESOR DESDE FIREBASE ---
    useEffect(() => {
        const cargarMisRecursos = async () => {
            if (!usuario?.uid) return; // <-- CORRECCIÓN: Usamos UID en vez de email
            try {
                // CORRECCIÓN: Buscamos por profesorUid, igual que hace tu dashboard
                const q = query(collection(db, 'resources'), where('profesorUid', '==', usuario.uid));
                const snapshot = await getDocs(q);
                const docs = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));

                // Filtramos solo los que sean minijuegos soportados
                const juegosSoportados = ['CAZABURBUJAS', 'APAREJADOS', 'PASAPALABRA', 'SOPA', 'WORDLE'];
                const filtrados = docs.filter(r => juegosSoportados.includes(r.tipoJuego));

                setRecursosPersonales(filtrados);
            } catch (error) {
                console.error("Error al cargar los recursos del profesor:", error);
            }
        };
        cargarMisRecursos();
    }, [usuario]);
    ...
    // --- NUEVO: MANEJADOR SELECCIÓN RECURSO MINIJUEGO ---
    const handleSeleccionRecurso = (idxPregunta, recursoId) => {
        const recursoSeleccionado = recursosPersonales.find(r => r.id === recursoId);
        const nuevas = [...datos.hojas];

        nuevas[hojaActiva].preguntas[idxPregunta] = {
            ...nuevas[hojaActiva].preguntas[idxPregunta],
            recursoId: recursoId,
            recursoObjeto: recursoSeleccionado, // Guardamos el objeto completo para inyectarlo
            app: recursoSeleccionado?.tipoJuego || 'DESCONOCIDO',
            hojaMinijuego: 'General', // Por defecto "General"
            tiempo: 60 // Por defecto 60 segundos
        };
        setDatos({ ...datos, hojas: nuevas });
    };
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center', background: '#f0f4f8', padding: '8px', borderRadius: '5px', flexWrap: 'wrap' }}>
                    <select value={tipo} onChange={(e) => updatePregunta(i, 'tipo', e.target.value)} style={{ ...inputStyleSmall, fontWeight: 'bold', minWidth: '140px', border: '1px solid #3498db', color: '#3498db' }}>
                        ...
                        {/* --- NUEVA OPCIÓN MINIJUEGO --- */}
                        <option value="MINIGAME">🎮 Minijuego</option>
                    </select>
                    {tipo !== 'PRESENTATION' && tipo !== 'MINIGAME' && (
                        <>
                ...
                {/* --- NUEVO BLOQUE: MINIJUEGO OLÍMPICO --- */}
                {tipo === 'MINIGAME' && (
                    <div style={{ border: '2px dashed #9b59b6', padding: '15px', borderRadius: '8px', background: '#f5eef8', marginTop: '5px' }}>
                        <div style={{ color: '#8e44ad', fontWeight: 'bold', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                            <Gamepad2 size={18} /> Inyectar Minijuego
                        </div>

                        <div style={{ marginBottom: '15px' }}>
                            <label style={styles.label}>1. Elige un recurso de tu biblioteca:</label>
                            <select
                                value={p.recursoId || ''}
                                onChange={e => handleSeleccionRecurso(i, e.target.value)}
                                style={{ ...styles.input, borderColor: '#9b59b6', fontWeight: 'bold', color: '#8e44ad' }}
                            >
                                <option value="">-- Cargar un recurso creado por mí --</option>
                                {recursosPersonales.map(r => (
                                    <option key={r.id} value={r.id}>
                                        {r.titulo} ({r.tipoJuego})
                                    </option>
                                ))}
                            </select>
                            {recursosPersonales.length === 0 && <p style={{ fontSize: '11px', color: '#e74c3c', marginTop: '5px' }}>No se han encontrado minijuegos en tu cuenta.</p>}
                        </div>
                        {p.recursoObjeto && (
                            <>
                                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '15px' }}>
                                    <div>
                                        <label style={styles.label}>2. ¿Qué hoja quieres usar?</label>
                                        <select
                                            value={p.hojaMinijuego || 'General'}
                                            onChange={e => updatePregunta(i, 'hojaMinijuego', e.target.value)}
                                            style={styles.input}
                                        >
                                            <option value="General">Mezcla General de todo el juego</option>
                                            {p.recursoObjeto.hojas?.map((h, idx) => (
                                                <option key={idx} value={h.nombreHoja}>{h.nombreHoja}</option>
                                            ))}
                                        </select>
                                    </div>
                                    <div>
                                        <label style={styles.label}>3. Tiempo total:</label>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                            <input
                                                type="number"
                                                value={p.tiempo || 60}
                                                onChange={e => updatePregunta(i, 'tiempo', parseInt(e.target.value))}
                                                style={{ ...styles.input, textAlign: 'center' }}
                                            />
                                            <span style={{ color: '#666', fontSize: '12px' }}>seg</span>
                                        </div>
                                    </div>
                                </div>

                                {/* --- NUEVO: SELECTOR BURBUJAS vs PIKATRON --- */}
                                {(p.recursoObjeto.tipoJuego === 'CAZABURBUJAS' || p.recursoObjeto.tipoJuego === 'PIKATRON') && (
                                    <div style={{ marginTop: '15px', background: '#f0f4f8', padding: '10px', borderRadius: '8px', border: '1px solid #bde0fe' }}>
                                        <label style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#2c3e50', display: 'block', marginBottom: '5px' }}>
                                            🎮 ¿Cómo quieres que lo jueguen?
                                        </label>
                                        <select
                                            value={p.app || p.recursoObjeto.tipoJuego}
                                            onChange={(e) => updatePregunta(i, 'app', e.target.value)}
                                            style={{ width: '100%', padding: '8px', borderRadius: '5px', border: '2px solid #3498db', fontWeight: 'bold', color: '#2c3e50', background: 'white' }}
                                        >
                                            <option value="CAZABURBUJAS">🔵 CazaBurbujas Clásico</option>
                                            <option value="PIKATRON">⚡ Pikatron Run (Runner)</option>
                                        </select>
                                    </div>
                                )}

                                {/* --- NUEVO: SELECTOR SOPA vs WORDLE --- */}
                                {(p.recursoObjeto.tipoJuego === 'SOPA' || p.recursoObjeto.tipoJuego === 'WORDLE') && (
                                    <div style={{ marginTop: '15px', background: '#f0f4f8', padding: '10px', borderRadius: '8px', border: '1px solid #bde0fe' }}>
                                        <label style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#2c3e50', display: 'block', marginBottom: '5px' }}>
                                            🧩 Elige el modo de juego:
                                    </label>
                                        <select
                                            value={p.app || 'SOPA'} // Por defecto Sopa
                                            onChange={(e) => updatePregunta(i, 'app', e.target.value)}
                                            style={{ width: '100%', padding: '8px', borderRadius: '5px', border: '2px solid #9b59b6', fontWeight: 'bold', color: '#2c3e50', background: 'white' }}
                                        >
                                            <option value="SOPA">🔡 Sopa de Letras</option>
                                            <option value="WORDLE">🟩 Wordle (Adivina la palabra)</option>
                                        </select>
                                    </div>
                                )}
        ...
        return (
            <div style={styles.mathBlock}>
                <div style={styles.mathHeader}>
                    <Calculator size={24} /> Configuración de Operaciones Aleatorias
                </div>
        ...
        <div style={styles.overlay}>
            ...
            <div style={styles.container}>
                ...
                <div style={styles.header}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, minWidth: 0 }}>
                        <h2 style={{ margin: 0, color: '#f1c40f', display: 'flex', alignItems: 'center', gap: '5px' }}>
                            <Trophy size={24} /> <span className="hide-mobile-xs">OLYMPIC LIVE</span>
                        </h2>
                        <input placeholder="Título del evento..." value={datos.titulo} onChange={e => setDatos({ ...datos, titulo: e.target.value })} style={styles.titleInput} />
                    </div>
                ...
                <div style={styles.body}>
                    ...
                    {/* --- NUEVO: INTERRUPTOR DE MATEMÁTICAS --- */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '15px', borderRadius: '10px', marginBottom: '20px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                        <div>
                            <div style={{ fontWeight: 'bold', fontSize: '1.1rem', color: '#2c3e50', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <Calculator size={20} color="#9C27B0" /> Generador Matemático Automático
                            </div>
                            <div style={{ fontSize: '0.9rem', color: '#7f8c8d' }}>Crear rondas de cálculo mental (Se mezclarán con tus minijuegos).</div>
                        </div>
                        <button
                            onClick={() => updateGlobalConfig('usarMates', !datos.config?.usarMates)}
                            style={{ ...styles.toggleBtn, background: datos.config?.usarMates !== false ? '#2ecc71' : '#ccc', justifyContent: datos.config?.usarMates !== false ? 'flex-end' : 'flex-start' }}
                        >
                            <div style={styles.toggleCircle}></div>
                        </button>
                    </div>

                    {/* MOSTRAR EL BLOQUE SOLO SI ESTÁ ACTIVO */}
                    {datos.config?.usarMates !== false && renderMathLiveBlock()}

                    <div style={{ borderBottom: '2px dashed #ccc', margin: '20px 0', paddingBottom: '10px', color: '#7f8c8d', fontStyle: 'italic', textAlign: 'center' }}>
                        A continuación, añade tus minijuegos y preguntas a la lista de reproducción:
                    </div>

                    {datos.hojas[hojaActiva]?.preguntas.length === 0 ? (
                        <div style={{ textAlign: 'center', color: '#95a5a6', marginTop: '20px' }}>
                            <p>No has añadido ninguna prueba a esta hoja.</p>
                            <button onClick={addPregunta} style={styles.addFirstBtn}>+ Añadir Prueba Olímpica</button>
                        </div>
                    ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '800px', margin: '0 auto' }}>
                                {datos.hojas[hojaActiva]?.preguntas.map((p, i) => (
                                    <div key={i} style={{ background: 'white', padding: '15px', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'flex-start', gap: '10px', borderLeft: `5px solid ${p.tipo === 'MINIGAME' ? '#9b59b6' : (p.tipo === 'PRESENTATION' ? '#95a5a6' : '#3498db')}` }}>
                                ...
                                <button onClick={addPregunta} style={styles.addMoreBtn}>+ AÑADIR OTRA PRUEBA</button>
                            </div>
                        )}
                </div>
                        ...
                        <div style={styles.configModal}>
                            ...
                            <div style={styles.configBody}>
                                ...
                                <div style={{ marginBottom: '10px' }}>
                                    ...
                                    <p style={{ fontSize: '11px', color: '#777' }}>Se sumarán a las operaciones y minijuegos.</p>
                                </div>
